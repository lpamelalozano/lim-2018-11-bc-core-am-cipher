...
No, no estamos hablando de un cifrado digital que no pueda romper la NSA, vamos a algo mucho más sencillo y es la posibilidad de enviar mensajes mediante WhatsApp u otra red social evitando que nadie que no queramos, pueda leer.
...
En esta fase usamos los datos coleccionados en la fase de investigación y descubrimiento buscando tendencias e insights valiosos para el desarrollo de Hidden App. Entendiendo los deseos y necesidades de nuestros usuarios, se desarrolló una plataforma simple y didactica, en la cual se permite cifrar / descifrar texto mediante una clave secreta, para luego poder copiar o enviarlo mediante WhatsApp u otra aplicación de mensajería.
Este resultado sirve para envíar mensajes con contenido delicado, ofrenciendo la seguridad de que nadie que no sepa la contraseña podrá leerlo y la comodidad de tener un pequeño código que podemos pegar donde sea y descifrar desde la plataforma de Hidden App.
...
Cada color provoca en las personas sensaciones determinadas asociadas a distintas emociones con la que se quiere llegar al usuario, influyendo en la manera en la que se sienten y comportan en la web.
...
Te invito a seguir los siguientes pasos para mejorar tu experiencia utilizando Hidden:
